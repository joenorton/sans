...
def test_check_script_happy_path():
    script = ("data mydata;\n" 
              ...
              "    by somevar;\n" 
              "run;\n" 
              "title \"hello\";") # This other statement should cause a refusal

    with pytest.raises(UnknownBlockStep) as exc_info:
        check_script(script, "test.sas", tables={"otherdata"})
...
def test_check_script_refuses_output_table_collision():
    ...
    with pytest.raises(UnknownBlockStep) as exc_info:
        check_script(script, "test.sas", tables={"initial_source"})
...
def test_check_script_happy_path_with_multiple_steps():
    ...
    irdoc = check_script(script, "test.sas", tables={"raw_data"})
